...
import { FileSpreadsheet, Image as ImageIcon, PhoneCall, RefreshCcw, Upload } from "lucide-react";
...
export default function ImportPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const data = await apiFetch<ImportResponse>("/customers/import", { method: "POST", body });
      setResult(data);
      setImportName("");
      setFile(null);
      await loadProgress();
  ...
  return (
    <AppShell>
      <PageHeading title="Nhập dữ liệu" subtitle="Tải lên tệp .xlsx và chỉ lấy các cột: tên công ty, tên người đứng đầu công ty, địa điểm, số điện thoại, thành phố." />
      ...
      {!user ? (
        ...
        <div className="space-y-5">
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
            <form onSubmit={submit} className="glass-card p-4">
              ...
              <label className="flex min-h-64 cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-cyan-300/[0.28] bg-cyan-300/[0.05] px-4 py-8 text-center transition hover:border-cyan-300/[0.55] hover:bg-cyan-300/[0.09] hover:shadow-glow">
                ...
                <span className="mt-1 text-xs text-slate-400">Hệ thống chỉ lấy tên công ty, người đứng đầu, địa điểm, số điện thoại, thành phố</span>
              ...
              <NeonButton disabled={!file || !importName.trim() || loading} className="mt-4">
                  ...
                  {result.errors.length ? (
                    <div className="rounded-lg border border-amber-300/[0.32] bg-amber-300/[0.10] p-3 leading-6 text-amber-100">
                      {result.errors.map((item) => (
                        <div key={`${item.row}-${item.message}`}>Dòng {item.row}: {item.message}</div>
                      ))}
          ...
          <ImportProgressPanel imports={imports} loading={loadingProgress} onRefresh={loadProgress} />
          ...
          <Modal open={showErrors} title="Chi tiết lỗi nhập dữ liệu" onClose={() => setShowErrors(false)}>
            <div className="max-h-[60vh] overflow-auto rounded-lg border border-amber-300/[0.24] bg-amber-300/[0.08] p-3 text-sm leading-6 text-amber-100">
              {result?.errors.map((item) => (
                <div key={`modal-${item.row}-${item.message}`}>Dòng {item.row}: {item.message}</div>
              ))}
            </div>
          </Modal>
        </div>
...
function ImportProgressPanel({ imports, loading, onRefresh }: { imports: ImportBatch[]; loading: boolean; onRefresh: () => void }) {
  return (
    <GlassCard>
                  ...
                  <TableBody>
                    {item.phones.length ? item.phones.map((phone) => (
                      <TableRow key={phone.customerId}>
                        ...
                        <Td className="text-slate-300">{phone.city || "-"}</Td>
                        <Td>
                          {phone.owner ? (
                            <div>
                        ...
                        <Td className="text-slate-300">{phone.calledBy?.name || "-"}</Td>
                        <Td>
                      ...
                      <tr><Td className="py-6 text-slate-400" colSpan={9}>Lô dữ liệu này chưa có số điện thoại hợp lệ.</Td></tr>
                    )}
                  </TableBody>
                </GlassTable>
              </div>
